...
def estimate_pitch_yaapt(path_to_audio):
    ...
    ende = time.time()
    print("Geschwindigkeit YAAPT: " + str(ende - start) + " Sekunden.")
...
def estimate_pitch_yin(path_to_audio):
    ...
    ende = time.time()
    print("Geschwindigkeit YIN: " + str(ende-start) + " Sekunden.")
...
def get_pitch_praat(textfile):
    pitch = np.genfromtxt(textfile, filling_values='nan')
    f0 = pitch[:, 1]
    times = pitch[:, 0]
    print(np.nanmean(f0, axis=0))
    return f0, times
